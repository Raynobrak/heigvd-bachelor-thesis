...
def plot_evaluation_results(rewards, durations, tunnels_angles, save_csv_path=CSV_FILE):
    ...
    print(len([d for d in durations if d == 30]))
    exit()
...
if __name__ == '__main__':
    ...
    if READ_FROM_FILE:
        rewards, durations, angles = load_evaluation_results()
        plot_evaluation_results(rewards, durations, angles)
        exit()
    else:
